...
This file took in 15 hours.
Learning Two.js was included in the time.

A crowd simulation using a simple hardcoded algorithm.
*/
...
class Agent extends PhysPoint {
    ...
    update (visible) {
        ...
        let weights =
            [{h: vto.getHeading(), m: 1 / vto.getMagnitude() ** 2}].concat(
            visible.map(visVec => {return {h: Angle.normalize(Math.PI + visVec.getHeading()), m: 1 / visVec.getMagnitude()}}))

        let tgr = Agent.calcAngle(weights)
        
        let dtz = Math.abs(this.vel.r ** 2 / (2 * MAX_ACC.r)) // angular distance to zero velocity
        let dtt = Angle.difference(tgr, this.pos.r) // angular distance to target
        
        if (
            Math.abs(dtt) < dtz ||
            Math.abs(Angle.difference(Angle.normalize(tgr + Math.PI), this.pos.r)) < dtz
        ) {
            this.acc.r = this.vel.r < 0 ? MAX_ACC.r : -MAX_ACC.r
        } else if (Angle.equals(tgr, this.pos.r)) {
            this.acc.r = 0
        } else this.acc.r = dtt >= 0 ? MAX_ACC.r : -MAX_ACC.r
            this.acc.setMagHead(this.fwd, this.pos.r)
